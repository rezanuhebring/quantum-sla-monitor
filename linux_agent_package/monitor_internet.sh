...
LOG_DATE=$(date -u +"%Y-%m-%dT%H:%M:%SZ"); AGENT_SOURCE_IP=$(ip route get 8.8.8.8 | awk '{print $7; exit}' 2>/dev/null || hostname -I | awk '{print $1}'); AGENT_HOSTNAME_LOCAL=$(hostname -s); declare -A results_map
...
if [ "$ENABLE_HTTP" = true ]; then
    ...
    if [ $? -eq 0 ]; then results_map[http_code]=$(echo "$curl_output_stats" | grep "http_code" | cut -d'=' -f2); results_map[http_time]=$(echo "$curl_output_stats" | grep "time_total" | cut -d'=' -f2 | sed 's/,/./'); if [[ "${results_map[http_code]}" -ge 200 && "${results_map[http_code]}" -lt 400 ]]; then results_map[http_status]="OK"; else results_map[http_status]="ERROR_CODE"; fi; else results_map[http_status]="FAILED_REQUEST"; fi
fi
