...
echo
print_info "The following project files and directories generated by the setup script will be removed:"
echo " - ${DOCKER_COMPOSE_FILE_NAME}"
echo " - Dockerfile"
echo " - nginx/ directory (if it exists)"
echo " - docker/ directory"
read -p "Proceed with deleting these items? (yes/no): " CONFIRM
if [ "$CONFIRM" == "yes" ]; then
    sudo rm -f "${DOCKER_COMPOSE_FILE_NAME}" Dockerfile
    sudo rm -rf nginx docker
    print_success "Project files and directories removed."
else
...
# --- Step 4: Remove SSL Certificate from Certbot (for Production installs) ---
echo
