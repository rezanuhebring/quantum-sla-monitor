...
# Layer 2: Install SQLite and related dev packages
RUN apt-get update -yqq && \
    apt-get install -y --no-install-recommends \
        sqlite3 \
        libsqlite3-dev \
...
# Layer 3: Install Zip and related dev packages
RUN apt-get update -yqq && \
    apt-get install -y --no-install-recommends \
        libzip-dev \
        zip \
...
# Layer 4: Install other CLI tools
RUN apt-get update -yqq && \
    apt-get install -y --no-install-recommends \
        curl \
        jq \
...
RUN echo "Attempting to install Ookla Speedtest CLI..." && \
    # Ensure curl and bash (from base image) are available
...
RUN echo "Layer 5: Speedtest OK"

# Layer 6: Install PHP extensions
RUN echo "Attempting to install PHP extensions..."
...
RUN echo "Layer 6: PHP extensions OK"
RUN apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* # Final cleanup
