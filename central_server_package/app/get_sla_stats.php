...
$EXPECTED_INTERVAL_MINUTES = 15; // How often you expect agents to check in. Used for advanced SLA calculation.
...
$response_data = [
    'isp_profiles' => [], 'all_agent_status' => [], 'current_isp_profile_id' => null, 'current_isp_name' => 'N/A',
    'target_sla_percentage' => 99.5, 'periods' => [], 'rtt_chart_data' => [], 'speed_chart_data' => [],
    'cumulative_ping_chart_data' => [], 'cumulative_speed_chart_data' => [], 'latest_check' => null, 
...
try {
    ...
    // --- ADVANCED SLA CALCULATION FOR DISPLAY ---
    if ($current_isp_profile_id) {
        $num_agents = 1;
        $sla_filter = " AND sm.isp_profile_id = {$current_isp_profile_id}";
    } else {
        $num_agents = (int)$db->querySingle("SELECT COUNT(*) FROM isp_profiles WHERE agent_type = 'ISP' AND is_active = 1") ?: 1;
        $sla_filter = " AND ip.agent_type = 'ISP'";
    }

    $total_possible = floor(($period_days * 1440) / $EXPECTED_INTERVAL_MINUTES) * $num_agents;
    $stmt = $db->prepare("SELECT COUNT(DISTINCT sm.id) FROM sla_metrics sm JOIN isp_profiles ip ON sm.isp_profile_id = ip.id WHERE sm.timestamp >= :start_date" . $sla_filter);
    $stmt->bindValue(':start_date', $start_date_iso);
    $met_intervals = (int)$stmt->execute()->fetchArray(SQLITE3_NUM)[0];
    
    $achieved_percentage = ($total_possible > 0) ? round(($met_intervals / $total_possible) * 100, 2) : 0.0;
    $response_data['periods'][] = ['label' => "Last {$period_days} Day(s)", 'total_intervals' => $total_possible, 'met_intervals' => $met_intervals, 'achieved_percentage' => $achieved_percentage, 'is_target_met' => ($achieved_percentage >= $response_data['target_sla_percentage'])];
