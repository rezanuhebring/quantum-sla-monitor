<?php
// get_sla_stats.php - FINAL PRODUCTION VERSION v3
// Enhanced with time period filtering, advanced SLA calculation, and summary data.
...
function parse_env_file($filepath) {
    ...
    foreach ($lines as $line) { if (empty(trim($line)) || strpos(trim($line), '#') === 0) continue; if (strpos($line, '=') !== false) { list($name, $value) = explode('=', $line, 2); $env_vars[trim($name)] = trim($value, " '\""); } }
    return $env_vars;
}
...
$response_data = [
    ...
    'rtt_chart_data' => [], 'speed_chart_data' => [], 'cumulative_ping_chart_data' => [], 'cumulative_speed_chart_data' => [], 'latest_check' => null, 'dashboard_refresh_interval_ms' => 60000, 'agent_stale_minutes' => 5
];
...
try {
    ...
    $response_data['agent_stale_minutes'] = (int)($config_values['AGENT_STALE_MINUTES'] ?? 5);
    ...
    $period_days_str = filter_input(INPUT_GET, 'period', FILTER_SANITIZE_STRING) ?: '1';
    $period_days = intval($period_days_str);

    $start_date_obj = new DateTime("now", new DateTimeZone("UTC"));
    $start_date_obj->modify("-{$period_days} days");
    ...
    if ($current_isp_profile_id) {
        ...
        // Fetch latest check and chart data
        ...
        $chart_query = $db->prepare("SELECT timestamp, avg_rtt_ms, avg_loss_percent, avg_jitter_ms, speedtest_download_mbps, speedtest_upload_mbps FROM sla_metrics WHERE isp_profile_id = :id AND timestamp >= :start_date ORDER BY timestamp ASC");
        ...
        // Get latest status for all agents for the summary lists
        $all_status_query = $db->query("SELECT sm.* FROM sla_metrics sm INNER JOIN (SELECT isp_profile_id, MAX(id) as max_id FROM sla_metrics GROUP BY isp_profile_id) latest ON sm.isp_profile_id = latest.isp_profile_id AND sm.id = latest.max_id JOIN isp_profiles ip ON sm.isp_profile_id = ip.id WHERE ip.is_active = 1");
        while ($status = $all_status_query->fetchArray(SQLITE3_ASSOC)) { $response_data['all_agent_status'][$status['isp_profile_id']] = $status; }
        
        // Fetch cumulative charts
    ...
    $expected_intervals = floor($total_minutes / $EXPECTED_INTERVAL_MINUTES);
    
    $sla_query_sql = "SELECT COUNT(DISTINCT id) FROM sla_metrics WHERE timestamp >= :start_date";
    if ($current_isp_profile_id) $sla_query_sql .= " AND isp_profile_id = :id";
    else $sla_query_sql .= " AND isp_profile_id IN (SELECT id FROM isp_profiles WHERE agent_type = 'ISP' AND is_active = 1)";

    $stmt = $db->prepare($sla_query_sql);
    $stmt->bindValue(':start_date', $start_date_iso);
    if ($current_isp_profile_id) $stmt->bindValue(':id', $current_isp_profile_id, SQLITE3_INTEGER);
    $met_intervals = (int)$stmt->execute()->fetchArray(SQLITE3_NUM)[0];
    
    // If we are looking at an individual agent, we can make the calculation more precise
    if($current_isp_profile_id) {
         $num_agents = 1;
    } else {
         $num_agents = (int) $db->querySingle("SELECT COUNT(*) FROM isp_profiles WHERE agent_type = 'ISP' AND is_active = 1");
         if ($num_agents == 0) $num_agents = 1; // Prevent division by zero
    }
    $total_possible_intervals = $expected_intervals * $num_agents;

    $achieved_percentage = ($total_possible_intervals > 0) ? round(($met_intervals / $total_possible_intervals) * 100, 2) : 0.0;
    $response_data['periods'][] = ['label' => "Last {$period_days} Day(s)", 'total_intervals' => (int)$total_possible_intervals, 'met_intervals' => (int)$met_intervals, 'achieved_percentage' => $achieved, 'is_target_met' => ($achieved >= $response_data['target_sla_percentage'])];
    
    $db->close();
} catch (Exception $e) {
    http_response_code(500);
    $response_data = ['error' => 'Server Error.', 'message' => $e->getMessage()];
