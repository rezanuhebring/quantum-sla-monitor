...
try {
    ...
    // Calculate SLA period based on view
    $sla_filter = $current_isp_profile_id ? " AND sm.isp_profile_id = {$current_isp_profile_id}" : " AND ip.agent_type = 'ISP'";
    $base_query_sql = "SELECT COUNT(*) as total, SUM(sla_met_interval) as met FROM sla_metrics sm JOIN isp_profiles ip ON sm.isp_profile_id = ip.id WHERE sm.timestamp >= :start_date" . $sla_filter;

    foreach (['1' => 'Last 24 Hours', '7' => 'Last 7 Days', '30' => 'Last 30 Days'] as $days => $label) {
        $start_date = (new DateTime("-{$days} days", new DateTimeZone("UTC")))->format("Y-m-d\TH:i:s\Z");
        $num_agents_in_calc = $current_isp_profile_id ? 1 : (int)$db->querySingle("SELECT COUNT(*) FROM isp_profiles WHERE agent_type = 'ISP' AND is_active=1");
        if($num_agents_in_calc == 0) $num_agents_in_calc = 1;

        $total_possible = floor(($days * 1440) / $EXPECTED_INTERVAL_MINUTES) * $num_agents_in_calc;
        $stmt = $db->prepare("SELECT SUM(sla_met_interval) as met_count FROM sla_metrics sm JOIN isp_profiles ip ON sm.isp_profile_id = ip.id WHERE sm.timestamp >= :start_date" . $sla_filter);
        ...
        $response_data['periods'][] = ['label' => $label, 'total_intervals' => $total_possible, 'met_intervals' => $met_intervals, 'achieved_percentage' => $achieved, 'is_target_met' => ($achieved >= $response_data['target_sla_percentage'])];
    }
