    ...
    <div class="dashboard-container">
        ...
        <div id="dashboard-content" class="hidden">
            <div id="summary-view" class="hidden">
                ...
                <div class="grid-container"><div class="card"><h3>System-Wide ISP SLA</h3><div id="overall-sla-periods"></div><div class="download-button-container"><a id="summary-csv-download" href="generate_csv.php" download="all_agents_summary.csv" class="download-button">💾 Download All Agent Status</a></div></div>
                <div class="card"><h3>System Info</h3><p><span>API Key:</span><strong id="api-key-display"></strong></p></div></div>
            ...
            <div id="detail-view" class="hidden">
                ...
                <div class="grid-container">
                    ...
                    <div id="wifi-status-card" class="card hidden"><h3>WiFi Status</h3><p><span>Status:</span><strong id="wifi-status"></strong></p><p><span>SSID:</span><strong id="wifi-ssid"></strong></p><p><span>Signal:</span><strong id="wifi-signal"></strong></p><p><span>Frequency:</span><strong id="wifi-frequency"></strong></p></div>
                </div>
                <div class="chart-card card" style="margin-top:20px;"><h3>Historical Performance (<span class="period-label"></span>)</h3><div class="chart-container"><canvas id="rttChart"></canvas></div><div class="chart-container" style="margin-top:20px;"><canvas id="speedTestChart"></canvas></div><div id="wifi-chart-container" class="chart-container hidden" style="margin-top:20px;"><canvas id="wifiSignalChart"></canvas></div></div>
            </div>
        </div>
    </div>
<script>
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderSummaryView = (data) => {
        ...
        setText('api-key-display', data.central_api_key);
    ...
    const renderDetailView = (data) => {
        ...
        // --- NEW: WiFi Chart & Info Handling ---
        const wifiCard = document.getElementById('wifi-status-card');
        const wifiChartContainer = document.getElementById('wifi-chart-container');
        const agentProfile = data.isp_profiles.find(p => p.id == data.current_isp_profile_id);

        if (agentProfile && agentProfile.agent_type === 'Client' && data.wifi_chart_data.length > 0) {
            wifiCard.classList.remove('hidden');
            wifiChartContainer.classList.remove('hidden');
            
            setText('wifi-status', latest?.wifi_status);
            setText('wifi-ssid', latest?.wifi_ssid);
            setText('wifi-signal', latest?.wifi_signal_strength_percent, ' %');
            setText('wifi-frequency', latest?.wifi_frequency_band);

            renderChart('wifiSignalChart', { 'WiFi Signal (%)': 'wifi_signal_strength_percent' }, data.wifi_chart_data, 'timestamp');
        } else {
            wifiCard.classList.add('hidden');
            wifiChartContainer.classList.add('hidden');
        }

        summaryView.classList.add('hidden'); detailView.classList.remove('hidden');
    };
    
    // UPDATED: More robust chart rendering function that updates instead of destroying
    const renderChart = (canvasId, seriesConfig, data, xKey) => {
        ...
        if (chartInstances[canvasId]) {
            ...
            chart.data.labels = data.map(d => d[xKey]);
