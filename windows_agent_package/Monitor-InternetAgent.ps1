...
# The entire script's logic is wrapped in a try/finally block to GUARANTEE cleanup.
try {
    ...
    $Results.detailed_health_summary = $HealthSummary
    $Results.current_sla_met_status = if ($SlaMetInterval -eq 1) { "MET" } else { "NOT_MET" }
