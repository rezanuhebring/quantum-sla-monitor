...
<#
.SYNOPSIS
    Internet SLA Monitoring Agent for Windows (PowerShell) - FINAL, CONSOLIDATED VERSION
.DESCRIPTION
    This is the definitive, hardened production script, incorporating all fixes and best practices.
    It uses a secure encrypted API key, robust object-based ping tests, and stable web requests.
.VERSION
    2.0.0
#>

# --- Configuration & Setup ---
# [UPDATED] Added agent versioning for easier tracking and management.
$AGENT_VERSION = "2.0.0"
...
# [UPDATED] Added path for the new secure API key file.
...
try {
    ...
    # [UPDATED] Log now includes the agent version.
    Write-Log -Message "Starting SLA Monitor Agent v$AGENT_VERSION (Type: $AGENT_TYPE)."
    
    # [UPDATED] New block to securely load the API key from the encrypted file.
    $DecryptedApiKey = $null
    if (Test-Path $EncryptedKeyPath) {
        try {
            $EncryptedString = Get-Content -Path $EncryptedKeyPath -Raw
            $DecryptedApiKey = (ConvertTo-SecureString -String $EncryptedString | ForEach-Object { [System.Runtime.InteropServices.Marshal]::PtrToStringAuto([System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($_)) })
        } catch {
            Write-Log -Level "ERROR" -Message "Failed to decrypt API key at '$EncryptedKeyPath'. Check file permissions and ensure it was created by the correct user (SYSTEM)."
        }
    }
    
    # [UPDATED] Fetch Profile now uses the robust -DisableKeepAlive switch.
    ...
    try {
        ...
        $WebRequest = Invoke-WebRequest -Uri $CentralProfileConfigUrl -Method Get -TimeoutSec 10 -UseBasicParsing -DisableKeepAlive
    ...
    # [UPDATED] agent_version is now part of the data payload.
    $Results = [ordered]@{
    ...
    # [DEFINITIVE UPDATE] PING TESTS replaced with the robust Test-Connection method.
    if ($ENABLE_PING) {
        Write-Log -Message "Performing robust ping tests using Test-Connection..."; 
        $allPingResults = @(); $totalPacketsSent = 0
        foreach ($pingTarget in $PING_HOSTS) {
            $totalPacketsSent += $PING_COUNT
            try { $pingResult = Test-Connection -TargetName $pingTarget -Count $PING_COUNT -ErrorAction Stop; $allPingResults += $pingResult; Write-Log -Message "Ping to ${pingTarget}: SUCCESS" } 
            catch { Write-Log -Level WARN -Message "Ping test to ${pingTarget} failed. Host may be unreachable." }
        }
        if ($allPingResults.Count -gt 0) {
            $rttValues = $allPingResults.ResponseTime; $averageRtt = ($rttValues | Measure-Object -Average).Average
            ...
            $Results.ping_summary = @{ status = "UP"; average_rtt_ms = [math]::Round($averageRtt, 2); average_packet_loss_percent = [math]::Round( (1 - ($allPingResults.Count / $totalPacketsSent)) * 100, 1 ); average_jitter_ms = [math]::Round($jitter, 3) }
        } else { $Results.ping_summary = @{ status = "DOWN"; average_packet_loss_percent = 100.0; average_jitter_ms = $null } }
    }

    # [UPDATED] HTTP TEST now uses the robust -DisableKeepAlive switch.
    ...
    if ($ENABLE_HTTP) { Write-Log "Performing HTTP check..."; try { $HttpTime = Measure-Command { $HttpResponse = Invoke-WebRequest -Uri $HTTP_CHECK_URL -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop -DisableKeepAlive }; $Results.http_check = @{ status = "OK"; response_code = $HttpResponse.StatusCode; total_time_s = [math]::Round($HttpTime.TotalSeconds, 3) } } catch { $Results.http_check = @{ status = "FAILED_REQUEST"; response_code = $null; total_time_s = $null } } }

    # [UPDATED] SPEEDTEST now includes silent-run flags.
    if ($ENABLE_SPEEDTEST) {
    ...
    # [PRESERVED & UPDATED] Your Health Summary logic is preserved. It now correctly uses the more accurate ping jitter.
    Write-Log "Calculating health summary...";
    $RttDegraded = Get-EffectiveThreshold $ProfileConfig "RTT_THRESHOLD_DEGRADED" "rtt_degraded" 100; $RttPoor = Get-EffectiveThreshold $ProfileConfig "RTT_THRESHOLD_POOR" "rtt_poor" 250; $LossDegraded = Get-EffectiveThreshold $ProfileConfig "LOSS_THRESHOLD_DEGRADED" "loss_degraded" 2; $LossPoor = Get-EffectiveThreshold $ProfileConfig "LOSS_THRESHOLD_POOR" "loss_poor" 10; $JitterDegraded = Get-EffectiveThreshold $ProfileConfig "PING_JITTER_THRESHOLD_DEGRADED" "ping_jitter_degraded" 30; $JitterPoor = Get-EffectiveThreshold $ProfileConfig "PING_JITTER_THRESHOLD_POOR" "ping_jitter_poor" 50; $DnsDegraded = Get-EffectiveThreshold $ProfileConfig "DNS_TIME_THRESHOLD_DEGRADED" "dns_time_degraded" 300; $DnsPoor = Get-EffectiveThreshold $ProfileConfig "DNS_TIME_THRESHOLD_POOR" "dns_time_poor" 800; $HttpDegraded = Get-EffectiveThreshold $ProfileConfig "HTTP_TIME_THRESHOLD_DEGRADED" "http_time_degraded" 1.0; $HttpPoor = Get-EffectiveThreshold $ProfileConfig "HTTP_TIME_THRESHOLD_POOR" "http_time_poor" 2.5; $DlDegraded = Get-EffectiveThreshold $ProfileConfig "SPEEDTEST_DL_THRESHOLD_DEGRADED" "speedtest_dl_degraded" 60; $DlPoor = Get-EffectiveThreshold $ProfileConfig "SPEEDTEST_DL_THRESHOLD_POOR" "speedtest_dl_poor" 30; $UlDegraded = Get-EffectiveThreshold $ProfileConfig "SPEEDTEST_UL_THRESHOLD_DEGRADED" "speedtest_ul_degraded" 20; $UlPoor = Get-EffectiveThreshold $ProfileConfig "SPEEDTEST_UL_THRESHOLD_POOR" "speedtest_ul_poor" 5
    ...
    elseif ($Results.dns_resolution.status -eq "FAILED" -or $Results.http_check.status -eq "FAILED_REQUEST") { $HealthSummary = "CRITICAL_SERVICE_FAILURE" }
        ...
        # Logic now uses the superior ping jitter from Test-Connection.
    ...
    # [DEFINITIVE UPDATE] Data submission block uses the secure key and robust Invoke-WebRequest.
    ...
    try {
        ...
        Invoke-WebRequest -Uri $CENTRAL_API_URL -Method Post -Body $JsonPayload -Headers $SubmitHeaders -TimeoutSec 60 -DisableKeepAlive | Out-Null
        
        Write-Log -Message "Data successfully submitted."
    } catch { 
        $ErrorMessage = "Failed to submit data. Error: $($_.Exception.Message)"
        if ($_.Exception.Response) { $ErrorMessage += " | HTTP Status: $($_.Exception.Response.StatusCode) - $($_.Exception.Response.StatusDescription)" }
