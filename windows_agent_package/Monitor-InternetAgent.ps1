...
.DESCRIPTION
    This script is the definitive, fully debugged agent. It uses dot-sourcing to load
    a standard .ps1 configuration file and includes all previous fixes.
#>

# --- Configuration & Setup ---
$AgentScriptDirectory = Split-Path -Parent $MyInvocation.MyCommand.Path
# FIX: Point to the correct .ps1 configuration file
...
function Write-Log {
    ...
    # FIX: Use the directly sourced variable
...
try {
    ...
    if (Test-Path $AgentConfigFile) {
        # FIX: Use dot-sourcing to load the script's variables into the current scope
        . $AgentConfigFile
    } else {
    ...
    if ($ENABLE_DNS) {
        Write-Log "Performing DNS resolution test..."; try { $DnsTime = Measure-Command { Resolve-DnsName -Name $DNS_CHECK_HOST -Type A -ErrorAction Stop -DnsOnly }; $Results.dns_resolution = @{ status = "OK"; resolve_time_ms = [int]$DnsTime.TotalMilliseconds } } catch { $Results.dns_resolution = @{ status = "FAILED"; resolve_time_ms = $null } }
    }
