...
.DESCRIPTION
    This script is the definitive, fully debugged agent. It uses the robust ping.exe
    command instead of Test-Connection to ensure reliability when run from Task Scheduler.
#>
...
try {
    ...
    try { $script:AgentConfig = Import-PowerShellDataFile -Path $AgentConfigFile }
    catch { $script:AgentConfig = @{ AGENT_IDENTIFIER = "UnconfiguredAgent" }; Write-Log -Level ERROR -Message "CRITICAL: Failed to load config '$AgentConfigFile'. Error: $($_.Exception.Message). Exiting."; exit 1 }
    ...
    $ProfileConfig = @{}; try { Write-Log -Message "Fetching profile from: $CentralProfileConfigUrl"; $WebRequest = Invoke-WebRequest -Uri $CentralProfileConfigUrl -Method Get -TimeoutSec 10 -UseBasicParsing; if ($WebRequest.StatusCode -eq 200) { $ProfileConfig = $WebRequest.Content | ConvertFrom-Json; Write-Log -Message "Successfully fetched profile config." } } catch { Write-Log -Level WARN -Message "Could not fetch profile config, will use local defaults. Error: $($_.Exception.Message)" }

    # --- Main Monitoring Logic ---
    $AgentSourceIpVal = (Invoke-RestMethod -Uri "https://api.ipify.org" -UseBasicParsing -ErrorAction SilentlyContinue) ?? "unknown"
    ...
    # --- PING TESTS (Using robust ping.exe) ---
    if ($ENABLE_PING) {
        Write-Log -Message "Performing ping tests using ping.exe..."
        $PingHosts = $AgentConfig.PING_HOSTS; $PingCount = $AgentConfig.PING_COUNT; $TotalRttSum = 0.0; $TotalLossSum = 0; $PingTargetsUp = 0;
        
        foreach ($pingTarget in $PingHosts) {
            try {
                # Force the command prompt to use the standard English code page for predictable output
                $pingOutput = cmd.exe /c "chcp 437 & ping -n $PingCount $pingTarget"
                if ($LASTEXITCODE -eq 0) {
                    Write-Log -Message "Ping to ${pingTarget}: SUCCESS"
                    $PingTargetsUp++
                    $LossLine = $pingOutput | Select-String -Pattern "Lost ="
                    $PacketLoss = ($LossLine -split 'Lost = ')[1] -split ' \(' | Select-Object -First 1
                    $RttLine = $pingOutput | Select-String -Pattern "Average ="
                    $AvgRtt = ($RttLine -split 'Average = ')[1] -replace 'ms', '' | ForEach-Object { $_.Trim() }

                    $TotalRttSum += [double]$AvgRtt
                    $TotalLossSum += [int]$PacketLoss
                } else {
                    Write-Log -Message "Ping to ${pingTarget}: FAIL"
                }
            } catch {
        ...
        if ($PingTargetsUp -gt 0) {
            ...
            $Results.ping_summary.average_packet_loss_percent = [math]::Round($TotalLossSum / $PingTargetsUp, 1)
            $Results.ping_summary.average_jitter_ms = $null # Jitter is not provided by ping.exe
        } else { $Results.ping_summary.status = "DOWN" }
    }

    # --- DNS, HTTP, Speedtest (Full Implementation) ---
    if ($ENABLE_DNS) { Write-Log "Performing DNS resolution test..."; try { $DnsTime = Measure-Command { Resolve-DnsName -Name $AgentConfig.DNS_CHECK_HOST -Type A -ErrorAction Stop -DnsOnly }; $Results.dns_resolution = @{ status = "OK"; resolve_time_ms = [int]$DnsTime.TotalMilliseconds } } catch { $Results.dns_resolution = @{ status = "FAILED"; resolve_time_ms = $null } } }
    ...
    function Get-Threshold($Profile, $Key, $Default) { if ($Profile.$Key -ne $null) { return [double]$Profile.$Key } else { return [double]$Default } }
    $RttDegraded = Get-Threshold $ProfileConfig "rtt_degraded" 100; $RttPoor = Get-Threshold $ProfileConfig "rtt_poor" 250; $LossDegraded = Get-Threshold $ProfileConfig "loss_degraded" 2; $LossPoor = Get-Threshold $ProfileConfig "loss_poor" 10;
    $JitterDegraded = Get-Threshold $ProfileConfig "ping_jitter_degraded" 30; $JitterPoor = Get-Threshold $ProfileConfig "ping_jitter_poor" 50; $DnsDegraded = Get-Threshold $ProfileConfig "dns_time_degraded" 300; $DnsPoor = Get-Threshold $ProfileConfig "dns_time_poor" 800;
    $HttpDegraded = Get-Threshold $ProfileConfig "http_time_degraded" 1.0; $HttpPoor = Get-Threshold $ProfileConfig "http_time_poor" 2.5; $DlDegraded = Get-Threshold $ProfileConfig "speedtest_dl_degraded" 60; $DlPoor = Get-Threshold $ProfileConfig "speedtest_dl_poor" 30;
    $UlDegraded = Get-Threshold $ProfileConfig "speedtest_ul_degraded" 20; $UlPoor = Get-Threshold $ProfileConfig "speedtest_ul_poor" 5;
