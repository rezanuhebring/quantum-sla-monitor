...
.DESCRIPTION
    ...
    - Corrected data submission using Invoke-WebRequest to prevent connection errors.
.VERSION
    1.2.1
#>
...
function Write-Log {
    ...
    $Timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss K"; $Identifier = if ($script:AGENT_IDENTIFIER) { $script:AGENT_IDENTIFIER } else { "WindowsAgent" }
    $LogEntry = "[$Timestamp] [$Level] [$Identifier] $Message"; try { Add-Content -Path $LogFile -Value $LogEntry -ErrorAction SilentlyContinue } catch {}
}

function Get-EffectiveThreshold {
    Param([Parameter(Mandatory=$true)]$ProfileConfig, [Parameter(Mandatory=$true)]$LocalConfigVarName, [Parameter(Mandatory=$true)]$ProfileConfigKey, [Parameter(Mandatory=$true)]$DefaultValue)
    if ($ProfileConfig -and $ProfileConfig.PSObject.Properties[$ProfileConfigKey]) { return [double]$ProfileConfig.$ProfileConfigKey }
    $LocalValue = Get-Variable -Name $LocalConfigVarName -Scope "Script" -ErrorAction SilentlyContinue
    if ($LocalValue -ne $null) { return [double]$LocalValue.Value }
    return [double]$DefaultValue
}

# --- Lock File Logic ---
if (Test-Path $LockFile) {
    if ((Get-Date) - (Get-Item $LockFile).CreationTime -gt [System.TimeSpan]::FromMinutes(10)) { Write-Log -Level WARN -Message "Stale lock file found. Removing it."; Remove-Item $LockFile -Force }
    else { Write-Log -Level INFO -Message "[LOCK] Previous instance is still running. Exiting."; exit 1 }
}
...
try {
    ...
    if (Test-Path $AgentConfigFile) { . $AgentConfigFile } 
    else { Write-Log -Level ERROR -Message "CRITICAL: Agent config file not found at '$AgentConfigFile'. Exiting."; exit 1 }
    ...
    if (Test-Path $EncryptedKeyPath) {
        ...
        } catch { Write-Log -Level "ERROR" -Message "Failed to decrypt API key at '$EncryptedKeyPath'. Check permissions and ensure it was created by the SYSTEM user." }
    }
    ...
    Write-Log -Message "Health Summary: $HealthSummary"; $Results.detailed_health_summary = $HealthSummary; $Results.current_sla_met_status = if ($SlaMetInterval -eq 1) { "MET" } else { "NOT_MET" };
    ...
    try {
        ...
        # [DEFINITIVE FIX] Use Invoke-WebRequest with -DisableKeepAlive to prevent all connection-related errors.
